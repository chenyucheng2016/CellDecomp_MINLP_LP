...
for i = 1:length(targets)
    ...
    for j = i+1:length(targets)
        vt2 = targets(j).loc;
        if is_collisionfree_2([1:length(recFur)],recFur, vt1,vt2) && is_collisionfree([1:length(walls)],walls, vt1,vt2)
            M_cost(i,j) = sqrt(sum((vt1-vt2).^2));
            M_cost(j,i) = sqrt(sum((vt1-vt2).^2));
        else
            M_cost(i,j) = 1e5;
            M_cost(j,i) = 1e5;
        end
...
for i = 1:length(targets)
    ...
    vt2 = vt_s;
    if is_collisionfree_2([1:length(recFur)],recFur, vt1,vt2) && is_collisionfree([1:length(walls)],walls, vt1,vt2)
        M_cost(i,31) = sqrt(sum((vt1-vt2).^2));
        M_cost(31,i) = sqrt(sum((vt1-vt2).^2));
    else
        M_cost(i,31) = 1e5;
        M_cost(31,i) = 1e5;        
    end
...
for i = 1 : length(Nodes_TSP)
    ...
    for j = i+1:length(Nodes_TSP)
        ...
        xy1 = mean(R1,2)';
        xy2 = mean(R2,2)';
        if abs(Nodes_TSP(j).theta - Nodes_TSP(i).theta) > pi/2
            continue
        end
        ...
        if abs(Nodes_TSP(i).theta - Nodes_TSP(j).theta) < 1e-6
            ...
            if sqrt(sum((xy1-xy2).^2))<2 && is_collisionfree([1:length(recFur)],recFur, xy1,xy2) && is_collisionfree([1:length(walls)],walls, xy1,xy2)
                is_conn = true;
            end
...
if Room ~= 4 && (trip_raw(length(trip_raw)) ~= last)
...
% 
% i = 1;
% while i<length(trip_raw)
%     if trip_raw(i)>ID_05pi
%         break
%     end
%     i = i+1;
% end
...
%         xy1 = [x1,y1];
%         xy2 = [x2,y2];
%         if is_collisionfree(obs_ind,recFur, xy1,xy2)
%             trip_raw(j+1:next-1) = [];
%         end
%         
%     end
...
for i = 1:length(trip_raw)
    ...
    node.ID = n1;
    node.theta = Nodes_TSP(i).theta;
    Nodes_path_all_smooth(ind) = {node};
    ind = ind+1;
...
[cost,trans_tot,r_tot] = cost_eval_3(w_trans,Nodes_path_all_smooth,pi/2);

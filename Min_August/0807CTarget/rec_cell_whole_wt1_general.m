...
for i = 1 : length(Nodes_TSP)
    ...
    for j = i+1:length(Nodes_TSP)
        ...
        if sqrt(sum((xy1-xy2).^2))<3 && is_collisionfree([1:length(walls)],walls, xy1,xy2) ... 
            && is_collisionfree([1:length(recFur)],recFur, xy1,xy2)  
            is_conn = true;
        end
...
Nodes_TSP(21).theta = pi;
Nodes_TSP(28).TID = -1;
v = [5.3;8.1]';
Nodes_TSP(55).vertices = {v}; 
Nodes_TSP(55).theta = 3*pi/2;
v = [5.4;6.9]';
Nodes_TSP(30).vertices = {v}; 
Nodes_TSP(30).theta = 3*pi/2;
...
[cost,trans_tot,r_tot] = cost_eval_4(w_trans,Nodes_path,2*pi)

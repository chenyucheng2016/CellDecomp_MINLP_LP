function [cost,trans_tot,r_tot] = cost_eval_5(w_trans,Nodes_path,theta_start)
...
% r_tot = r_tot + abs(atan2(norm(cross(u,v)),dot(u,v)));
...
for i = 1:length(Nodes_path)-2
    ...
    theta_2 = Nodes_path{i+1}.theta;
    v2_norm = [cos(theta_2), sin(theta_2), 0];
    ...
    v = xy3 - xy2;
    d_theta = 0;
%     d_theta = d_theta + abs(atan2(norm(cross(u,v)),dot(u,v)));
%     d_theta = d_theta + abs(atan2(norm(cross(u,v2_norm)),dot(u,v2_norm)));
    d_theta = min(abs(Nodes_path{i+1}.theta-Nodes_path{i}.theta),pi/2);
    
    cost = cost + w_rotat * d_theta;

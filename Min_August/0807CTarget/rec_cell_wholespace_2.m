...
for i = 1 : length(Nodes_TSP)
    ...
    for j = i+1:length(Nodes_TSP)
        ...
        if xy1(1)>=Rm_x_min && xy1(2)>=Rm_y_min && xy2(1)>=Rm_x_min && xy2(2)>=Rm_y_min
            % in room 2
            if rect_adj(R1, R2)
            ...
            if abs(Nodes_TSP(i).theta - Nodes_TSP(j).theta) < 1e-6
                ...
                if sqrt(sum((xy1-xy2).^2))<3 && is_collisionfree([1:length(recFur)],recFur, xy1,xy2) && is_collisionfree([1:length(walls)],walls, xy1,xy2)
                    is_conn = true;
                end
...
for i = 1:size(conn_pair,1)
    ...
    keySet = [keySet,st];
    valueSet = [valueSet,w_trans* sqrt((x1-x2)^2 + (y1-y2)^2) + w_rotat*abs(d_theta)];

...
%             plot(robot.loc(1), robot.loc(2),'bs','MarkerFaceColor','r','MarkerSize',8);
        ...
        function boolean = inCFree_v1(robot,obstacles)
...
%              plot([robotXs   robotXs(1)],[robotYs   robotYs(1)],'g');
        ...
        % given a circular zoom, representing robot rotating, detect if cir
        % in C-free, if in Cfree, robot can rotate any degree at this
        % location
        function boolean = inCFree(robot,robotCir,obstacles)
...
%             disp(radtodeg(difAngle))
        ...
        % Determine if a target in the FOV (Updated in CTarget_v8)
        ...
        % Determine if a target in the FOV  (rotating camera, used before CTarget_v7)
        function flag = ptInFOV_rot_camera(robot, pt,obstacles)
            flag = false;
...
%                 end
                ...
                % Determine if any of targets is in the FOV --> can define
        % C-target
        function boolean = inCTarget2d(robot,targetSet,obstacles)
            boolean = false;
            for i = targetSet
                if ptInFOV_rot_camera(robot,i,obstacles)
                    boolean = true;
                    break
                end
%                 if ptInFOV(robot,i)

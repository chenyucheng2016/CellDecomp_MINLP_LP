...
function varargout = tspofs_ga(varargin)
    ...
    % Added by Min: record the best results track
    optRoutes = [];
    ...
    mins_num = 1; % addded by Min to record history path
    for iter = 1:numIter
        ...
        if minDist < globalMin
            globalMin = minDist;
            optRoute = pop(index,:);
            distHistory_2(mins_num) = minDist;
            optRoutes = [optRoutes; optRoute];
            mins_num = mins_num + 1;
            if showProg
                % Plot the Best Route
                rte = [1 optRoute];

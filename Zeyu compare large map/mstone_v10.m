...
%{
Update: 
1. combine EER & RBB-->hybrid sampling
2. 
Problem:
...
% global obstacles
% obstacles = {walls,recFur};
% B3dtoB2d();

% ms = genMS(targets);
% nRBB = 10;
ms_uniform = genRBB(targets,0,12); %nRBB,nUnif
...
% C = load('C_300'); C = C.C;
 ...
 ms_EER = sampleEER(C,8);
% %  ms_EER = [];
%  ms_RBB = genRBB(targets,3,4);
% % ms_RBB = [];
 ms = [ms_uniform ms_EER];

all = {targets,walls,recFur,cirFur,UGV,ms};

%  plotEERmap(C,all);
% figure;
% plot3Dworld(all,0.01); 
% view(2);
...
function ms = genRBB(targets,nRBB,nUnif)
global A2d
n = length(A2d);
sigma = round(n*0.08);
msRBB = [];  msUnif = [];

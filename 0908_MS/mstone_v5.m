...
%{
Update: 
1. add the connectivity calculation for milestone, dist < threashold

Problem:

ToDos:
1. implement A* or what
...
global A2d
n = length(A2d);
numMS = 50;  % number of total milestone
...
for i = 1:n
    ...
    for j = 1:n
        ...
        if ifConnect(ms1,ms2,obstacles) && distPts(ms1.loc(1),ms1.loc(2),ms2.loc(1),ms2.loc(2)) < maDist
            mtrx(i,j) = 1;
            plot([ms1.loc(1)  ms2.loc(1)],[ms1.loc(2)  ms2.loc(2)],'k-');
        end
    end
